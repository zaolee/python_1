...
# 8 bit = 1byte. 1024 Byte(210 byte) = 1KB. 1024 KB(210 KB)= 1MB. 1024 MB(210 MB)= 1GB. 1024 GB(210 GB)= 1TB
# 필요한 저장 공간을 MB 단위로 바꾸어 출력한다. 단, 소수점 셋째 자리에서 반올림하여 둘째 자리까지 출력한다.

w, h, b = input("w, h, b의 값을 순서대로 입력하세요!!").split()
...
# 기본 소수점 출력 자릿수 지정 : %.nf -> n번째 자릿수까지 반올림한 값을 출력하고 싶어요!!
# %라는 문자 자체를 출력하고 싶을 때 %%를 써야 출력돼.
# 예) 소수 2번째 자리까지 출력 ㄱㄱ -> print("%.2f"%변수(값)